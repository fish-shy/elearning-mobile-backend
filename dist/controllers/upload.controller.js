...
exports.uploadController = {
    async uploadSingle(req, res) {
        try {
            ...
            const publicUrl = await (0, uploadConfig_1.uploadToGCS)(req.file, folder);
    ...
    async uploadMultiple(req, res) {
        try {
            ...
            const uploadResults = await Promise.all(files.map(async (file) => {
                const publicUrl = await (0, uploadConfig_1.uploadToGCS)(file, folder);
                const metadata = await fileMetadata_service_1.fileMetadataService.create({
    ...
    async uploadProfileImage(req, res) {
        try {
            ...
            const publicUrl = await (0, uploadConfig_1.uploadToGCS)(req.file, 'profile-images');
            res.status(201).json({
    ...
    async uploadCourseImage(req, res) {
        try {
            ...
            const publicUrl = await (0, uploadConfig_1.uploadToGCS)(req.file, 'course-images');
            res.status(201).json({
    ...
    async uploadLessonContent(req, res) {
        try {
            if (!req.file) {
                return res.status(400).json({ error: 'No file provided' });
            }
            const publicUrl = await (0, uploadConfig_1.uploadToGCS)(req.file, 'lesson-content');
            const fileMetadata = await fileMetadata_service_1.fileMetadataService.create({
                fileName: req.file.originalname,
                fileSize: req.file.size,
                fileType: req.file.mimetype,
    ...
    async uploadSubmission(req, res) {
        try {
            ...
            const publicUrl = await (0, uploadConfig_1.uploadToGCS)(req.file, 'submissions');
            const fileMetadata = await fileMetadata_service_1.fileMetadataService.create({
                fileName: req.file.originalname,
                fileSize: req.file.size,
                fileType: req.file.mimetype,
    ...
    async deleteFile(req, res) {
        try {
            ...
            const file = await fileMetadata_service_1.fileMetadataService.findById(id);
            if (!file) {
